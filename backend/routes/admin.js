...
router.get('/students', protect, adminOnly, async (req, res) => {
  try {
    const { year, semester, batch, search, course, enrollmentStatus } = req.query;
    const query = {};
    ...
    // Filter by selected course (takes priority over enrollment status)
    if (course) {
      query.selectedCourse = course;
    } else if (enrollmentStatus === 'enrolled') {
      // Only apply enrollment status filter if no specific course filter
      query.selectedCourse = { $ne: null };
    } else if (enrollmentStatus === 'not-enrolled') {
      query.selectedCourse = null;
    }
...
router.get('/stats', protect, adminOnly, async (req, res) => {
  try {
    ...
    const enrolledStudents = await Student.countDocuments({ selectedCourse: { $ne: null } });
    const notEnrolledStudents = totalStudents - enrolledStudents;
    const totalCourses = await Course.countDocuments();
    const courses = await Course.find().select('name enrolledStudents capacity semester');
    const fullCourses = courses.filter(c => c.enrolledStudents.length >= c.capacity).length;
    ...
    const courseStats = courses.map(c => ({
      ...
      capacity: c.capacity,
      fillPercentage: Math.round((c.enrolledStudents.length / c.capacity) * 100)
    }));
...
// Get students grouped by course (for enrollments view)
router.get('/enrollments', protect, adminOnly, async (req, res) => {
  try {
    ...
    const courses = await Course.find(courseQuery)
      .populate('enrolledStudents', 'name rollNumber year semester batch');
    ...
    // Also get students with no course selected
    const unassignedQuery = { selectedCourse: null };
    if (semester) unassignedQuery.semester = semester;
    const unassignedStudents = await Student.find(unassignedQuery)
      .select('name rollNumber year semester batch');

    res.json({ enrollments, unassignedStudents });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
